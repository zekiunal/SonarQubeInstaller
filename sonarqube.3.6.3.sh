...
####################################################################################
# @package sonarqube 3.6.3 server initialize
...
####################################################################################
# Install Maven 3.0.5 Install
####################################################################################
# Download Maven

wget http://ftp.itu.edu.tr/Mirror/Apache/maven/maven-3/3.0.5/binaries/apache-maven-3.0.5-bin.tar.gz
# Unzip 
...
echo '<?xml version="1.0" encoding="UTF-8"?>' >> /usr/local/maven/conf/settings.xml
echo '<settings xmlns="http://maven.apache.org/SETTINGS/1.0.0"' >> /usr/local/maven/conf/settings.xml
echo '          xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"' >> /usr/local/maven/conf/settings.xml
echo '          xsi:schemaLocation="http://maven.apache.org/SETTINGS/1.0.0 http://maven.apache.org/xsd/settings-1.0.0.xsd">' >> /usr/local/maven/conf/settings.xml
echo '' >> /usr/local/maven/conf/settings.xml
echo '   <pluginGroups></pluginGroups>' >> /usr/local/maven/conf/settings.xml
echo '    <proxies></proxies>' >> /usr/local/maven/conf/settings.xml
echo '    <servers></servers>' >> /usr/local/maven/conf/settings.xml
echo '    <mirrors></mirrors>' >> /usr/local/maven/conf/settings.xml
echo '    <profiles>' >> /usr/local/maven/conf/settings.xml
echo '		<profile>' >> /usr/local/maven/conf/settings.xml
echo '			<id>sonar-profile</id>' >> /usr/local/maven/conf/settings.xml
echo '			<properties>' >> /usr/local/maven/conf/settings.xml
echo '				<sonar.host.url>http://localhost:9000/</sonar.host.url>' >> /usr/local/maven/conf/settings.xml
echo '				<sonar.jdbc.url>jdbc:mysql://localhost:3306/sonar?useUnicode=true&amp;characterEncoding=utf8</sonar.jdbc.url>' >> /usr/local/maven/conf/settings.xml
echo '				<sonar.jdbc.driver>com.mysql.jdbc.Driver</sonar.jdbc.driver>' >> /usr/local/maven/conf/settings.xml
echo '				<sonar.jdbc.username>sonar</sonar.jdbc.username>' >> /usr/local/maven/conf/settings.xml
echo '				<sonar.jdbc.password>'${password}'</sonar.jdbc.password>' >> /usr/local/maven/conf/settings.xml
echo '				<sonar.language>php</sonar.language>' >> /usr/local/maven/conf/settings.xml
echo '			</properties>' >> /usr/local/maven/conf/settings.xml
echo '		</profile>' >> /usr/local/maven/conf/settings.xml
echo '	</profiles>' >> /usr/local/maven/conf/settings.xml
echo '	<activeProfiles>' >> /usr/local/maven/conf/settings.xml
echo '		<activeProfile>sonar-profile</activeProfile>' >> /usr/local/maven/conf/settings.xml
echo '	</activeProfiles>' >> /usr/local/maven/conf/settings.xml
echo '</settings>' >> /usr/local/maven/conf/settings.xml


####################################################################################
# Install Sonar 3.6.3 Install
####################################################################################
if [ ! -L /etc/init.d/sonar ]
    then
    	wget http://dist.sonar.codehaus.org/sonar-3.6.3.zip
	unzip sonar-3.6.3.zip
	rm -f sonar-3.6.3.zip
	mv sonar-3.6.3 /usr/local
	ln -s /usr/local/sonar-3.6.3/ /usr/local/sonar
	...
	echo 'sonar.jdbc.password:                       '${password}'' >> /usr/local/sonar/conf/sonar.properties
